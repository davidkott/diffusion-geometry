...
def calc_t_emb(ts, t_emb_dim):
    ...
    t_emb = torch.exp(torch.arange(half_dim) * -t_emb).to(ts.device)
    t_emb = ts * t_emb
...
def rescale(x):
    ...
    """
    return (torch.clamp(x,min=-1,max=1)+1)/2
...
def sampling(net, size, T, Alpha, Alpha_bar, Sigma):
    ...
    with torch.no_grad():
        for t in range(T-1,-1,-1):
            ...
            ts = t * torch.ones( size[0] ).cuda()
            epsilon_theta = net(x,ts)
            x = (x - (1-Alpha[t])/torch.sqrt(1-Alpha_bar[t]) * epsilon_theta) / torch.sqrt(Alpha[t])
            if t > 0:
                x = x + Sigma[t] * std_normal(size).cuda()
    return x

def training_loss(net, loss_fn, T, X, Alpha_bar,rank):
    """
    Compute the loss_fn (default is \ell_2) loss of (epsilon - epsilon_theta)
    """
    B, C, H, W = X.shape
    ts = torch.randint(T, size=(B,1,1,1)).to(rank)
    z = std_normal(X.shape).to(rank)
    xt = torch.sqrt(Alpha_bar[ts]) * X + torch.sqrt(1-Alpha_bar[ts]) * z
    epsilon_theta = net(xt, ts.view(B))
